<script lang="ts">
  ...
  const links = [
    ...
    {
      name: "博客",
      path: siteConfig.blogUrl || "/blog",
      external: !!siteConfig.blogUrl,
    },
...
>
    ...
    <div class="flex items-center justify-between h-16">
      ...
      <div class="flex items-center justify-between w-full md:w-auto">
        ...
        <div class="shrink-0 pointer-events-auto mr-4">
          <a
            href={currentPath === "/" ? null : "/"}
            class="group flex items-center justify-center rounded-full bg-white/5 backdrop-blur-sm dark:bg-white/5 border border-white/20 shadow-sm hover:shadow-lg hover:bg-white/15 dark:hover:bg-white/15 transition-all duration-300 w-10 h-10 p-0 {isLinkActive(
              '/',
              currentPath,
            )
        ...
        <div class="md:hidden pointer-events-auto flex items-center gap-4">
          ...
          {:else}
            ...
            <div class="w-32.5 h-10.5 opacity-0"></div>
          {/if}
          <button
      ...
      <div class="hidden md:block pointer-events-auto">
        <div
          class="relative flex items-center bg-white/5 dark:bg-white/5 backdrop-blur-sm rounded-full p-1 border border-white/5 shadow-sm"
        >
          {#each links as link, i}
            <a
              href={link.path}
              target={link.external ? "_blank" : "_self"}
              class="relative px-6 py-2 rounded-full text-sm font-bold transition-colors duration-300
                    {isLinkActive(link.path, currentPath)
                ? 'text-white'
                : 'text-gray-800 dark:text-gray-300 hover:text-black dark:hover:text-white'}"
            >
              {#if isLinkActive(link.path, currentPath)}
                <span
                  class="absolute inset-0 bg-linear-to-r from-blue-600/50 to-blue-500/50 rounded-full shadow-md"
                  style="view-transition-name: nav-active;"
                ></span>
              {/if}
              <span
                class="relative z-10"
                style="view-transition-name: nav-link-{i};">{link.name}</span
              >
            </a>
          {/each}
        </div>
  ...
  {#if isOpen}
    <div
      ...
      class="md:hidden absolute top-20 left-4 right-4 pointer-events-auto bg-white/50 dark:bg-white/5 backdrop-blur-sm border border-white/20 rounded-2xl shadow-lg z-50 overflow-hidden"
    >
        ...
        {#each links as link, i}
          <a
            ...
            class="block px-3 py-2 rounded-md text-base font-medium transition-colors
                {isLinkActive(link.path, currentPath)
              ? 'bg-blue-50 text-blue-600 dark:bg-white/10 dark:text-blue-300'
